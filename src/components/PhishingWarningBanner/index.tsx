...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://cheeseswap.financial - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/cheeseswap.financial)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text) => (
        <Text
          small
          as="span"
          bold={text === 'https://cheeseswap.financial'}
          color={text === 'https://cheeseswap.financial' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
